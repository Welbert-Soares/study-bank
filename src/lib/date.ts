export function getBrazilianDate(date?: Date): Date {
  ...
  const brasiliaDate = new Date(d.toLocaleString('en-US', { timeZone: brasiliaTimezone }))
...
export function getStartOfDay(date: Date): Date {
  ...
  const startDay = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), 3, 0, 0, 0))
  return startDay
}
...
export function getEndOfDay(date: Date): Date {
  ...
  const endDay = new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), 26, 59, 59, 999))
  return endDay
}
